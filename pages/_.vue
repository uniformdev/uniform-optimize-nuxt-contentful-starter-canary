...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      // $uniformOptimizeTrackerAnalytics is injected by the custom analytics plugin (see ~/plugins/analytics.client.js).
      if (this.$uniformOptimizeTrackerAnalytics) {
        if (typeof this.$uniformOptimizeTrackerAnalytics.page === 'function') {
